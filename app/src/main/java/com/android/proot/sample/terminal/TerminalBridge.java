...
public class TerminalBridge implements TerminalSessionClient, TerminalViewClient {
    ...
    public interface SessionCallback {
        void onTitleChanged(TerminalSession session, String title);
        void onSessionFinished(TerminalSession session, int exitCode);
    }
    ...
    public void sendHome(TerminalSession session) {
        sendBytes(session, new byte[]{27, '[', 'H'});
    }

    public void sendEnd(TerminalSession session) {
        sendBytes(session, new byte[]{27, '[', 'F'});
    }
    ...
    public void onTitleChanged(TerminalSession changedSession) {
        if (callback != null && changedSession != null) {
            ...
            mainHandler.post(() -> callback.onTitleChanged(changedSession, title));
        }
    }
    ...
    public void onSessionFinished(TerminalSession finishedSession) {
        if (callback != null && finishedSession != null) {
            ...
            mainHandler.post(() -> callback.onSessionFinished(finishedSession, exitCode));
        }
    }
    ...
    public void onCopyAllTextToClipboard(TerminalSession session) {
        if (session == null || session.getEmulator() == null) return;
        try {
            String text = session.getEmulator().getScreen().getTranscriptText();
            if (text != null && !text.trim().isEmpty()) {
                onCopyTextToClipboard(session, text.trim());
            } else {
                Toast.makeText(context, "终端暂无内容可复制", Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            Log.w(TAG, "Failed to copy transcript", e);
        }
    }
