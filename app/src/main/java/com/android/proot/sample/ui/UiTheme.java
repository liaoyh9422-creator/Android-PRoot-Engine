...
public class UiTheme {
        ...
        view.setOnTouchListener((v, event) -> {
            switch (event.getActionMasked()) {
                case android.view.MotionEvent.ACTION_DOWN:
                    v.animate().scaleX(0.94f).scaleY(0.94f).setDuration(50).start();
                    try {
    ...
    /** Create state list drawable with pressed state feedback */
    public static android.graphics.drawable.StateListDrawable createTactileKeyDrawable(
            Context ctx, String normalBg, String pressedBg, String strokeColor, int strokeWidthDp, int radiusDp) {
        android.graphics.drawable.StateListDrawable sld = new android.graphics.drawable.StateListDrawable();
        GradientDrawable pressed = roundRect(ctx, pressedBg != null ? pressedBg : C_BORDER, strokeColor, strokeWidthDp, radiusDp);
        GradientDrawable normal = roundRect(ctx, normalBg != null ? normalBg : C_SURFACE_ALT, strokeColor, strokeWidthDp, radiusDp);
    ...
    /**
     * Styles an action tile widget with tactile feedback and clickability.
     */
    public static void styleActionTile(Context ctx, View tile) {
        if (tile == null) return;
        tile.setClickable(true);
        tile.setFocusable(true);
        applyTactileFeedback(tile);
    }
}
